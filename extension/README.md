...
Connect now registers the selected tab with the local service before running model or page-control scans. A background/incompletely loaded AI page can be connected without a page reload; its controls are inspected afterward, and jobs still wait for a ready prompt editor before sending. A failed local handshake is reported instead of showing a false connected state.
