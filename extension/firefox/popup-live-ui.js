(() => {
  ...
  function bind() {
    ...
    $('#preferences-button').addEventListener('click', () => togglePopover($('#preferences-popover'), $('#preferences-button')));
    ...
    document.addEventListener('click', (event) => {
      if (!event.target.closest('.popover, #connection-button, #scope-button, #page-menu-button, #session-policy-button, #preferences-button')) closePopovers();
    });
  ...
  function renderLanguages(){const list=$('#language-list');list.replaceChildren(...[['en','English'],['de','Deutsch']].map(([code,name])=>{const button=document.createElement('button');button.className='language-option';button.type='button';button.dataset.locale=code;button.setAttribute('role','radio');button.setAttribute('aria-checked',String(code===locale));button.innerHTML=`<span class="language-code">${code.toUpperCase()}</span><span class="language-name"><strong>${name}</strong><span>${code==='de'?'Deutsch verwenden':'Use English'}</span></span><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="m6 12 4 4 8-9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`;button.addEventListener('click',()=>saveUI({uiLocale:code}));return button;}));}
  async function saveUI(patch){await api.storage.local.set(patch);closePopovers();await refresh(false);}
  function applyUI(saved){document.documentElement.lang=locale;$('#language-code').textContent=locale.toUpperCase();$('#alpha-tooltip-title').textContent=locale==='de'?'Alpha-Vorschau':'Alpha preview';$('#alpha-tooltip-copy').textContent=locale==='de'?'Frühe Version. Oberfläche und Verhalten können sich noch ändern.':'Early release. Interface and behavior may still change.';$$('.language-option').forEach((button)=>button.setAttribute('aria-checked',String(button.dataset.locale===locale)));const pref=['system','light','dark'].includes(saved.uiTheme)?saved.uiTheme:'system',theme=pref==='system'?(matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):pref;document.documentElement.dataset.theme=theme;document.documentElement.dataset.themePref=pref;document.documentElement.dataset.text=['compact','comfortable','large'].includes(saved.uiTextSize)?saved.uiTextSize:'comfortable';$$('.theme-option').forEach((b)=>b.classList.toggle('active',b.dataset.themeOption===pref));$$('.appearance-option').forEach((b)=>b.classList.toggle('active',b.dataset.size===document.documentElement.dataset.text));}
