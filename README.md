...
For local selector troubleshooting, run `contextbridge browser inspect --config ./config.yml`. The paired extension reports a small, read-only DOM snapshot for each selected tab: prompt and send controls, file inputs (including hidden ones), relevant tool controls, response-image counts, and image progress. The local status does not contain the prompt value, chat text, file contents, cookies, or full HTML. With multiple tabs, specify `--tab ID`; the snapshot is refreshed with the extension heartbeat and is not sent to the relay.
