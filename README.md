...
2. Open a ChatGPT or Gemini chat and click **Attach current page** in the popup; the same button becomes **Detach current page** for that page, including when another row is selected below. **Attach fresh ChatGPT + Gemini chats** chooses only tabs confirmed empty by two page checks. Existing conversations always need an explicit attach click. The tab list also has **Allow this tab**, **Detach this tab**, and **Detach all tabs** for other pages; merely highlighting a row never grants access. Detaching revokes future jobs, though work already submitted to a provider may still finish.
3. Choose **Test profile**, then **Start browser bridge**. Gemini model choices are discovered automatically when an attached tab is idle with an empty composer; the scan button refreshes them on demand. The optional auto-attach setting adds only new, empty ChatGPT/Gemini chats while connected and only for origins already permitted by the browser. Turn it off whenever you prefer exclusively manual attachment. Each attached tab is one serial browser slot.
