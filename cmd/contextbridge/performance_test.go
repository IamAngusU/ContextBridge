...
func TestMeasurePerformanceFileSizesCountsOnlyRequestedTrees(t *testing.T) {
	root := t.TempDir()
	...
	for directory, size := range map[string]int{"chromium": 11, "firefox": 13, "src": 7} {
		path := filepath.Join(extension, directory)
		if err := os.MkdirAll(path, 0700); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(path, "asset"), bytes.Repeat([]byte{'x'}, size), 0600); err != nil {
			t.Fatal(err)
		}
	}
	files, warnings, err := measurePerformanceFileSizes(binary, extension)
	if err != nil {
		t.Fatal(err)
	}
	if len(warnings) != 0 || files.BinaryBytes != 17 || files.ChromiumExtensionBytes != 11 || files.FirefoxExtensionBytes != 13 || files.ExtensionSourceBytes != 7 {
