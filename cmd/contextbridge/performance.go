...
type performanceFileSizes struct {
	...
	ExtensionRoot          string `json:"extension_root,omitempty"`
	ChromiumExtensionBytes int64  `json:"chromium_extension_bytes,omitempty"`
	FirefoxExtensionBytes  int64  `json:"firefox_extension_bytes,omitempty"`
	ExtensionSourceBytes   int64  `json:"shared_extension_source_bytes,omitempty"`
}
...
func performanceCommand(args []string) error {
	...
	extensionRoot := flags.String("extension-root", "", "extension root containing chromium, firefox, and optionally src")
	...
	report := performanceReport{
		...
		Excludes: []string{
			...
			"browser rendering and website latency",
		...
		Unavailable: []performanceUnavailableMetric{
			{Name: "ram_per_active_inference_job", Reason: "the benchmark deliberately starts no provider, browser, or model inference; process-wide browser/model memory cannot be attributed honestly to one ContextBridge job"},
	...
	files, warnings, err := measurePerformanceFileSizes(*binaryPath, *extensionRoot)
	if err != nil {
		return err
	}
...
func measurePerformanceFileSizes(requestedBinary, requestedExtension string) (performanceFileSizes, []string, error) {
	...
	extension := strings.TrimSpace(requestedExtension)
	if extension == "" {
		for _, candidate := range []string{
			filepath.Join(filepath.Dir(binary), "extension"),
		...
		if extension == "" {
			warnings = append(warnings, "extension root not found; pass --extension-root to measure installed extension files")
	...
	if info, err := os.Stat(extension); err != nil || !info.IsDir() {
		if requestedExtension != "" {
			return result, warnings, errors.New("--extension-root must name an existing directory")
		}
		warnings = append(warnings, "extension root not found; pass --extension-root to measure installed extension files")
		return result, warnings, nil
	}
	result.ExtensionRoot = extension
	for name, destination := range map[string]*int64{
		"chromium": &result.ChromiumExtensionBytes,
		"firefox":  &result.FirefoxExtensionBytes,
		"src":      &result.ExtensionSourceBytes,
	} {
		value, err := regularFileBytes(filepath.Join(extension, name))
		if err != nil {
			if errors.Is(err, os.ErrNotExist) {
				warnings = append(warnings, name+" extension directory not found")
				continue
			}
			return result, warnings, err
		}
...
func representativeHeartbeatMetrics() ([]performanceHeartbeat, error) {
	...
	browserPayload, err := json.Marshal(bridge.BrowserClientStatus{
		Connected: true, State: "waiting", SelectorsReady: true, ExtensionVersion: "measurement", Browser: "chromium", ActiveTabs: 1,
		Tabs: []bridge.BrowserTabStatus{{
			ID: 1, Origin: "https://chatgpt.com", Profile: "chatgpt", State: "waiting", CurrentModel: "measured-model", DOMStatus: "ready",
			DOM: &bridge.BrowserDOMSnapshot{
				CapturedAt: fixedTime, PageVisibility: "visible",
				Inputs:         []bridge.BrowserDOMControl{{Tag: "div", Role: "textbox", Visible: true}},
				Submit:         []bridge.BrowserDOMControl{{Tag: "button", AriaLabel: "Send", Visible: true}},
				AssistantTurns: 1,
			},
		}},
	})
	...
	return []performanceHeartbeat{
		metric("browser_one_idle_tab", "representative JSON body; actual size scales with attached tabs and bounded diagnostics", len(browserPayload)),
...
func printPerformanceReport(output io.Writer, report performanceReport) {
	...
	fmt.Fprintln(output, "Provider/model inference, browser rendering, Internet latency, and transport framing are excluded.")
	fmt.Fprintln(output)
	...
	fmt.Fprintf(output, "  binary                         %s\n", formatBytes(uint64(report.Resources.Files.BinaryBytes)))
	if report.Resources.Files.ChromiumExtensionBytes > 0 {
		fmt.Fprintf(output, "  Chromium extension              %s\n", formatBytes(uint64(report.Resources.Files.ChromiumExtensionBytes)))
	}
	if report.Resources.Files.FirefoxExtensionBytes > 0 {
		fmt.Fprintf(output, "  Firefox extension               %s\n", formatBytes(uint64(report.Resources.Files.FirefoxExtensionBytes)))
	}
	idle := report.Resources.IdleRelay
	fmt.Fprintf(output, "  benchmark + idle relay window   %s\n", time.Duration(idle.DurationMilliseconds)*time.Millisecond)
	...
	fmt.Fprintf(output, "  fresh Bolt growth               %s for %d cancelled jobs · normalized %s/1000\n",
		formatBytes(uint64(database.GrowthBytes)), database.Jobs, formatBytes(uint64(database.GrowthPer1000JobBytes)))
	for _, heartbeat := range report.Resources.Heartbeat {
		fmt.Fprintf(output, "  %-30s %d B/heartbeat · %d B/min · %s/hour at %.0fs\n",
			...
			formatBytes(uint64(heartbeat.PayloadBytesPerHour)), float64(heartbeat.IntervalMilliseconds)/1000)
	}
