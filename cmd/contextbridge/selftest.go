...
const selftestLocal = "local"

var selftestProfilePattern = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9._-]{0,79}$`)
...
func clusterSelftestCommand(args []string) error {
	...
	providersRaw := flags.String("providers", "local", "comma-separated checks: local and any configured adapter profile IDs")
	...
	image := flags.Bool("image", false, "also request and verify one real image artifact from an adapter (explicit provider-cost opt-in)")
	imageProfile := flags.String("image-profile", "", "adapter profile for --image; inferred when exactly one adapter profile is requested")
	...
	selectedImageProfile, err := selectSelftestImageProfile(providers, *imageProfile, *image)
	if err != nil {
		return err
	}
	...
	if *run {
		...
		if selftestHasAdapter(providers) {
			mode += " · fresh adapter sessions only"
		}
	}
	...
	if *run {
		...
		if *image {
			fmt.Printf(" + 1 image request via %s", selectedImageProfile)
		}
		fmt.Println()
	}
	...
	if *image {
		...
		imageTarget := selftestTarget{}
		for _, target := range plan.Targets {
			if strings.EqualFold(target.Kind, selectedImageProfile) {
				imageTarget = target
				break
			}
		}
		fmt.Printf("| → %s image artifact\n", selectedImageProfile)
		jobStarted := time.Now()
		jobCtx, cancelJob := context.WithTimeout(rootCtx, *jobTimeout)
		output, job, err := runSelftestJob(jobCtx, cfg, token, imageTarget, selftestJobSpec{
		...
		cancelJob()
		if err != nil {
			return fmt.Errorf("adapter profile %s image check: %w", selectedImageProfile, err)
		}
		...
		if verifiedImages < 1 {
			return fmt.Errorf("adapter profile %s image check completed without a transferred image file", selectedImageProfile)
		}
		...
		if len(paths) == 0 {
			return fmt.Errorf("adapter profile %s image check returned no savable image bytes", selectedImageProfile)
		}
...
func parseSelftestProviders(raw string) ([]string, error) {
	...
	for _, item := range strings.Split(raw, ",") {
		...
		case "":
		default:
			value = strings.TrimPrefix(value, "adapter:")
			if !selftestProfilePattern.MatchString(value) || value == selftestLocal {
				return nil, fmt.Errorf("invalid adapter profile %q; use local or a safe adapter profile ID", strings.TrimSpace(item))
			}
			wanted[value] = true
		}
	}
	providers := make([]string, 0, len(wanted))
	if wanted[selftestLocal] {
		providers = append(providers, selftestLocal)
		delete(wanted, selftestLocal)
	}
	profiles := make([]string, 0, len(wanted))
	for profile := range wanted {
		profiles = append(profiles, profile)
	}
	sort.Strings(profiles)
	providers = append(providers, profiles...)
	if len(providers) == 0 {
		return nil, errors.New("--providers must include local or at least one adapter profile ID")
	}
	return providers, nil
}

func selftestHasAdapter(providers []string) bool {
	for _, provider := range providers {
		if provider != selftestLocal {
			return true
		}
	}
	return false
...
func selectSelftestImageProfile(providers []string, requested string, enabled bool) (string, error) {
	if !enabled {
		if strings.TrimSpace(requested) != "" {
			return "", errors.New("--image-profile requires --image")
		}
		return "", nil
	}
	requested = strings.TrimPrefix(strings.ToLower(strings.TrimSpace(requested)), "adapter:")
	if requested != "" {
		if !selftestProfilePattern.MatchString(requested) || requested == selftestLocal {
			return "", fmt.Errorf("invalid --image-profile %q", requested)
		}
		if !containsSelftestProvider(providers, requested) {
			return "", fmt.Errorf("--image-profile %s must also be listed in --providers", requested)
		}
		return requested, nil
	}
	profiles := make([]string, 0, len(providers))
	for _, provider := range providers {
		if provider != selftestLocal {
			profiles = append(profiles, provider)
		}
	}
	if len(profiles) != 1 {
		return "", errors.New("--image requires exactly one adapter profile in --providers or an explicit --image-profile")
	}
...
func selftestTargetLabel(kind string) string {
	if kind == selftestLocal {
		return "▣LOC local generation"
	}
	return "◆ADP " + kind + " endpoint"
}
	...
	for {
		...
		if ctx.Err() != nil {
			...
			if errors.Is(ctx.Err(), context.DeadlineExceeded) {
				return selftestPlan{}, fmt.Errorf("readiness timeout: %s. Start the listed targets, connect the adapter, or change --providers", lastMessage)
			}
		...
		case <-ctx.Done():
			clearInteractive()
			if errors.Is(ctx.Err(), context.DeadlineExceeded) {
				return selftestPlan{}, fmt.Errorf("readiness timeout: %s. Start/attach the listed targets, connect the adapter, or change --providers", lastMessage)
			}
			return selftestPlan{}, ctx.Err()
		case <-ticker.C:
		}
...
func selftestTimingSummary(job cluster.Job, wall time.Duration) string {
	...
	if job.Usage.QueueMS > 0 {
		parts = append(parts, "queue "+formatSelftestPreciseDuration(durationFromUint64Milliseconds(job.Usage.QueueMS)))
	}
	if job.Usage.ComputeMS > 0 {
		parts = append(parts, "compute "+formatSelftestPreciseDuration(durationFromUint64Milliseconds(job.Usage.ComputeMS)))
	}
	...
	for _, kind := range providers {
		...
		for _, node := range nodes {
			...
			if kind == selftestLocal {
				...
				if !containsFolded(node.Capabilities.Providers, "adapter") {
					continue
				}
				providerSeen = true
				for _, session := range node.Capabilities.AdapterSessions {
					...
					if !selftestNodeCanSchedule(node, cluster.Requirements{Task: "generation", Provider: "adapter", AdapterProfile: kind}) {
						busySeen = true
						continue
					}
		...
		if len(candidates) == 0 {
			...
			case kind != selftestLocal && profileSeen && busySeen:
				plan.Missing = append(plan.Missing, kind+" endpoint is busy or rate-limited")
			case kind != selftestLocal && providerSeen:
				plan.Missing = append(plan.Missing, "make an idle "+kind+" endpoint available through the ContextBridge adapter")
			case staleSeen:
...
func buildSelftestRequest(target selftestTarget, spec selftestJobSpec, runID string) (cluster.SubmitRequest, error) {
	...
	if target.Kind != selftestLocal {
		provider = "adapter"
		profile = target.Kind
		model = ""
		// The self-test never reuses an existing operator session. Every adapter
		// request asks for a fresh, isolated, ephemeral session.
	...
	requirements := cluster.Requirements{Task: "generation", Provider: provider, AdapterProfile: profile, Model: model, SessionID: sessionID, PreferredNodes: []string{target.NodeID}}
	if provider == "adapter" && metadata["contextbridge_new_session"] == true {
		requirements.AdapterFreshSession = true
		requirements.AdapterEphemeralSession = metadata["contextbridge_new_session_per_job"] == true
	}
