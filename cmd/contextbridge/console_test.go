...
func TestFetchConsoleStatusUsesReadOnlyAuthenticatedRequest(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"version":"v0.test","queued":2,"completed":3,"browser":{"connected":true,"active_tabs":1,"tabs":[{"id":7,"profile":"chatgpt","current_model":"GPT-5.6 Sol"}]},"metrics":{"jobs_total":4,"jobs_failed":1},"runtime":{"hardware":{"gpus":[{"name":"RTX","utilization_percent":12}]}}}`))
	}))
	defer server.Close()
	...
	if snapshot.Version != "v0.test" || snapshot.Queued != 2 || !snapshot.BrowserConnected || snapshot.Tabs[0].CurrentModel != "GPT-5.6 Sol" || snapshot.GPU != "RTX" {
