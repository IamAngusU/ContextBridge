...
func runDoctor(path string) doctorReport {
	...
	if err := doctorGET(client, localURL+"/v1/status", cfg.Server.Token, &status); err != nil || !status.OK {
		...
		if len(ready) == 0 {
			add("fail", "default route", strings.Join(details, ", "), "Start Ollama/a managed engine, or teach and connect an AI browser tab with the bundled extension.")
		} else {
...
func checkClusterFiles(cfg config.Config, add func(string, string, string, string)) {
	...
	if cfg.Cluster.Worker.Enabled {
		...
		client := &http.Client{Timeout: 4 * time.Second}
		if err := doctorGET(client, strings.TrimRight(cfg.Cluster.Worker.RelayURL, "/")+"/health", "", nil); err != nil {
			add("fail", "worker relay connection", err.Error(), "Check relay_url, TLS, DNS, and whether the relay is running.")
		} else {
			add("ok", "worker relay connection", "relay is reachable", "")
		}
	} else {
		add("warn", "cluster worker", "disabled on this device", "Use `contextbridge cluster configure --mode worker|all` when this device should contribute CPU, GPU, or a browser tab.")
	}
}
...
func doctorProviderState(provider string, status doctorStatus) string {
	if provider == "browser" {
		...
		if !status.Browser.SelectorsReady {
