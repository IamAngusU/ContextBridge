...
func TestReadPoolDisplayUsesReadOnlyCredentialAndMapsSafeFields(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`[{"id":"node_abc","name":"workstation","connected":true,"public_key":"not-for-terminal","capabilities":{"running":1,"max_concurrent":4}},{"id":"node_def","name":"server","connected":false,"capabilities":{"max_concurrent":2}}]`))
	}))
	defer server.Close()
