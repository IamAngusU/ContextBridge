...
func TestMCPStdioLifecycleAndBoundedTools(t *testing.T) {
	...
	if len(listed) != 3 {
		t.Fatalf("expected exactly three bounded tools, got %#v", listed)
	}
	for index, expected := range []string{"contextbridge.status", "contextbridge.submit", "contextbridge.result"} {
...
func TestMCPProtocolVersionNegotiation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if tools, ok := responseResult(t, responses[1])["tools"].([]interface{}); !ok || len(tools) != 3 {
...
func TestMCPSubmitSchemaDescribesSafeNativeJobFields(t *testing.T) {
	...
	input := tools[1]["inputSchema"].(map[string]interface{})
	job := input["properties"].(map[string]interface{})["job"].(map[string]interface{})
	properties := job["properties"].(map[string]interface{})
	for _, name := range []string{"prompt", "text", "session_id", "browser_profile", "model", "reasoning", "image_base64", "output"} {
	...
	if _, ok := metadataProperties["contextbridge_new_chat"]; !ok {
		t.Fatalf("safe fresh-chat metadata is missing from bounded MCP schema: %#v", metadataProperties)
	}
}
...
func TestMCPStatusRedactsEndpointCredentialsAndURLs(t *testing.T) {
	...
	tab := structured["browser"].(map[string]interface{})["tabs"].([]interface{})[0].(map[string]interface{})
	if _, exists := tab["session_key"]; exists {
		t.Fatalf("opaque browser session key survived MCP scrubbing: %#v", tab)
	}
}
...
func TestMCPMarksNormalizedProviderFailureAsToolError(t *testing.T) {
	local := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"job":{"id":"job-failed"},"status":"completed","output":{"mode":"text","provider":"contextbridge","error":"browser_lease_lost"}}`))
	}))
	defer local.Close()
	server := newTestMCPServer(local.URL, "token")
	responses := runMCPTranscript(t, server,
	...
	structured := result["structuredContent"].(map[string]interface{})
	if structured["output"].(map[string]interface{})["error"] != "browser_lease_lost" {
...
func TestMCPRejectsInternalBrowserTabRoutingBeforeLocalSubmit(t *testing.T) {
	var requests atomic.Int32
	local := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		requests.Add(1)
		w.WriteHeader(http.StatusInternalServerError)
	...
	responses := runMCPTranscript(t, server,
		initializeMCPTestMessage(),
		`{"jsonrpc":"2.0","method":"notifications/initialized"}`,
		`{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"contextbridge.submit","arguments":{"job":{"prompt":"steer internally","contextbridge_browser_tab_id":42,"output":{"mode":"text"}}}}}`,
	)
	if requests.Load() != 0 {
		t.Fatalf("internal browser tab field reached the local service %d times", requests.Load())
	}
	result := responseResult(t, responses[1])
	if result["isError"] != true {
	...
	content := result["content"].([]interface{})[0].(map[string]interface{})["text"].(string)
	if !strings.Contains(content, "contextbridge_browser_tab_id") {
