...
func TestPowerShellTabExpansionTracksTrailingSpaceAndOptionPosition(t *testing.T) {
	...
	tests := []struct {
		...
		{"integration targets", "contextbridge integrate ", []string{"openai", "mcp", "relay", "ui"}, []string{"status", "serve"}},
		...
		{"UI integration options", "contextbridge integrate ui ", []string{"--config", "--write-env", "--subject", "--lifetime-hours"}, []string{"--show-token", "--live", "--providers"}},
...
func TestBashCompletionOffersRootCommandFlagsAtCurrentWord(t *testing.T) {
	...
	tests := []struct {
		...
		{"integrate ui", "contextbridge integrate ui --", 3, []string{"--config", "--write-env", "--subject", "--lifetime-hours"}, []string{"--show-token", "--live", "--providers"}},
