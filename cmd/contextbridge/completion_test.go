...
func TestPowerShellTabExpansionTracksTrailingSpaceAndOptionPosition(t *testing.T) {
	...
	tests := []struct {
		...
		{"relay integration options", "contextbridge integrate relay ", []string{"--config", "--json", "--write-env", "--subject", "--groups", "--lifetime-hours"}, []string{"--show-token", "--live"}},
...
func TestBashCompletionOffersRootCommandFlagsAtCurrentWord(t *testing.T) {
	...
	tests := []struct {
		...
		{"integrate relay", "contextbridge integrate relay --", 3, []string{"--config", "--json", "--write-env", "--subject", "--groups", "--lifetime-hours"}, []string{"--show-token", "--live"}},
