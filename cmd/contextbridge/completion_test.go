...
func TestPowerShellTabExpansionTracksTrailingSpaceAndOptionPosition(t *testing.T) {
	...
	tests := []struct {
		...
		{"integration targets", "contextbridge integrate ", []string{"openai", "mcp"}, []string{"status", "serve"}},
		{"OpenAI integration options", "contextbridge integrate openai ", []string{"--config", "--json", "--show-token", "--write-env"}, []string{"status", "serve"}},
...
func TestBashCompletionOffersRootCommandFlagsAtCurrentWord(t *testing.T) {
	...
	tests := []struct {
		...
		{"integrate openai", "contextbridge integrate openai --", 3, []string{"--config", "--json", "--show-token", "--write-env"}, nil},
