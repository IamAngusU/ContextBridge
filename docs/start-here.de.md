...
> **Versionshinweis, bitte lesen:** Der Installer lädt die [neueste veröffentlichte GitHub-Release](https://github.com/IamAngusU/ContextBridge/releases/latest), nicht automatisch den neuesten Commit auf `main`. Dieser Guide ist für **v0.5.80** vorbereitet. Der einfache Texttest unten funktioniert mit älteren 0.5.x-Ständen; das vollständige Demo-Runbook mit geprüftem Portable-Pool, frischen Chats, verifizierter Bildübergabe und den aktuellen Sicherheitsgrenzen benötigt mindestens **v0.5.80**. Prüfe vor der Demo mit `contextbridge version`, was wirklich installiert ist. Falls die verlinkte neueste Release noch darunter liegt, ist v0.5.80 noch nicht öffentlich veröffentlicht – ein Commit oder lokal geladener Ordner ist keine Release.
