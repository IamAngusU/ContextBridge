...
The runtime samples hardware capabilities, inspects Ollama, supervises configured `llama.cpp` processes, and publishes one status snapshot to the CLI and dashboard. It can also resolve a configured engine through a passively discovered portable-resource marker; the stable pack and endpoint IDs survive drive-letter or mount-path changes, while an absent pack simply makes that engine unavailable. Sealed or checksum-verified resource trees can remain unchanged by using a bounded volume-root sidecar with a confined relative target. Discovery is bounded, never executes pack code, and accepts only loopback endpoints. Engines bind to localhost unless a reviewed OpenAI-compatible engine explicitly enables HTTPS remote egress. Ordered route fallbacks determine which engine is tried next.

The Ollama provider sends a trusted task wrapper and optional image to a configured local model. The `llama_cpp` provider uses the local OpenAI-compatible chat and embedding endpoints. An explicit `openai_compatible` engine can call a reviewed loopback or HTTPS provider with an exact model and operator-declared capabilities; non-loopback egress requires both `remote: true` and a resolved secret. In the opposite direction, the authenticated `/openai/v1` compatibility subset maps client model IDs to existing routes. The browser provider leases work to selected extension tabs. Each worker publishes a bounded per-tab record of profile, state, visible current selection, model choices, reasoning levels, fresh-chat capability, and—only for clustered sessions with a permanent saved conversation—an opaque producer-scoped selector. Chat text, tab titles, public session names, and conversation URLs are excluded, and public node responses redact the selector. When a browser job fixes both profile and model, one ready waiting tab or exact matching owned session must satisfy both instead of the scheduler combining evidence from different tabs on the same node. Hosted web-chat inference still runs at that provider; the extension replaces a separate API integration, not the provider's compute. A cluster can route parallel browser jobs across several tabs on one PC and across multiple paired PCs.
