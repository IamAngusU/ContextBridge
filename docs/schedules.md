...
You can also write `{{previous.text}}`, `{{previous.json}}`, or `{{previous.artifact_names}}` in a follow-up prompt. ContextBridge replaces the marker with a reference and puts the actual prior AI output in the next job's **untrusted submitted-content section**, not its trusted instructions. An unknown or empty referenced field fails that step without sending it. An optional `use_previous_artifact: "image"` or `"file"` attaches the first prior artifact with actual bytes and a matching SHA-256 digest to a browser follow-up. A verified image can also be handed to a subsequent Ollama or llama.cpp step; arbitrary local-model file handoff is intentionally unsupported. The previous step must request `output.artifacts: true`. The handoff revalidates the declared media type and, where supported, its byte signature, plus decoded size and SHA-256; it records the immediate source job as provenance, keeps the schedule session, and accepts at most 8 MiB because that is the image-input limit. Artifact-carrying steps are pinned to the validated provider, so a route fallback cannot silently drop the file and still report success. URL-only, cross-run, oversized, mislabeled, or tampered files are not silently fetched or guessed. Ollama image handoff also requires `providers.ollama.images: true`; automatic Ollama selection requires verified vision capability. An explicit Ollama model or llama.cpp engine remains an operator-selected local override and may return a provider error if it lacks visual input support. A failed or interrupted follow-up never automatically replays preceding steps. The dashboard offers one follow-up; the authenticated API/CLI supports up to four.
