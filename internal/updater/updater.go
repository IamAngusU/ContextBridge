...
func (m *Manager) status(state State) Status {
	...
	// dashboard or CLI toggle to opt in. An explicit false remains an admin lock.
...
func scheduleRetry(state *State) {
	// State is persisted on disk. Treat negative/corrupt/overflow-adjacent
	// counters as the maximum bounded backoff instead of incrementing first:
	// MaxInt+1 would wrap and feed a negative shift count below.
	if state.Failures < 0 || state.Failures >= 8 {
		state.Failures = 8
	} else {
		state.Failures++
	}
