...
func cpuFrequencyMHz(ctx context.Context) int {
	if runtime.GOOS == "linux" {
		if raw, err := os.ReadFile("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq"); err == nil {
			...
			if mhz, ok := boundedFrequencyMHz(value, 1000); ok {
				return mhz
			}
		}
	...
	if runtime.GOOS == "darwin" {
		...
		value, _ := strconv.ParseUint(strings.TrimSpace(string(raw)), 10, 64)
		mhz, _ := boundedFrequencyMHz(value, 1_000_000)
		return mhz
	}
	return 0
}

func boundedFrequencyMHz(value, divisor uint64) (int, bool) {
	if value == 0 || divisor == 0 {
		return 0, false
	}
	mhz := value / divisor
	// One million MHz is already far outside plausible hardware and also
	// keeps the conversion portable on 32-bit builds.
	if mhz == 0 || mhz > 1_000_000 {
		return 0, false
	}
	return int(mhz), true // #nosec G115 -- mhz is proven <= 1,000,000 above.
}
	...
	if runtime.GOOS == "linux" {
		if raw, err := os.ReadFile("/proc/uptime"); err == nil {
			...
			if len(fields) > 0 {
				...
				if parseErr == nil && !math.IsNaN(value) && !math.IsInf(value, 0) && value > 0 && value <= math.MaxUint64 {
					return uint64(value)
				}
			}
		}
		...
		if raw, err := exec.CommandContext(ctx, "sysctl", "-n", "kern.boottime").Output(); err == nil {
			...
			if index := strings.Index(text, "sec ="); index >= 0 {
				...
				if len(fields) > 0 {
					...
					now := time.Now().Unix()
					if boot > 0 && now > boot {
						// #nosec G115 -- the preceding comparison proves the delta is positive.
						return uint64(now - boot)
					}
				}
			}
		}
