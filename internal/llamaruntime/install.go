...
func Current(directory string) string {
	...
	if readErr != nil || closeErr != nil || len(raw) > 4096 {
		return ""
	}
	...
	if stat, statErr := os.Stat(resolved); statErr == nil && stat.Mode().IsRegular() {
		return resolved
	}
	return ""
}
...
func extractZip(path, target string) error {
	...
	for _, item := range reader.File {
		...
		entrySize, err := validatedZipEntrySize(item.UncompressedSize64, extracted)
		if err != nil {
...
func validatedZipEntrySize(size uint64, extracted int64) (int64, error) {
	// Validate in the unsigned domain before converting. A crafted ZIP header
	// can report a value above MaxInt64; converting first would wrap it negative
	// and make subsequent arithmetic dependent on overflow behaviour.
	if extracted < 0 || extracted > maximumRuntimeExtractedBytes {
		return 0, errors.New("invalid extracted byte count")
	}
	remaining := uint64(maximumRuntimeExtractedBytes - extracted) // #nosec G115 -- both operands are validated non-negative and ordered above.
	if size > uint64(maximumRuntimeEntryBytes) || size > remaining {
		return 0, errors.New("ZIP entry exceeds extraction limits")
	}
