...
func TestWorkerRelayParallelCapacityEndToEnd(t *testing.T) {
	...
	local := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		case "/v1/status":
			writeJSON(w, http.StatusOK, map[string]interface{}{
				...
				"routes": map[string]interface{}{
					"default": map[string]interface{}{"task": "generation", "model": "adapter-endpoint", "provider": "adapter"},
				},
				"adapter": map[string]interface{}{"connected": true, "ready": true, "active_endpoints": 1},
		...
		case "/v1/jobs":
			...
			if json.NewDecoder(req.Body).Decode(&payload) == nil && payload.Provider == "adapter" && strings.HasPrefix(payload.ID, "cluster-") {
			...
			writeJSON(w, http.StatusOK, map[string]interface{}{
				...
				"output": map[string]interface{}{"mode": "text", "text": "ok", "model": "adapter-endpoint"},
				"status": "completed",
			})
		default:
	...
	for index := 0; index < 3; index++ {
		job, createErr := relay.store.CreateJob(SubmitRequest{
			Requirements: Requirements{Task: "generation", Provider: "adapter", SessionID: fmt.Sprintf("parallel-%d", index)},
...
func TestLocalOutputErrorRecognizesSubmissionAndDirectOutput(t *testing.T) {
	...
	if got := localOutputError([]byte(`{"mode":"text","error":"adapter_automation_error"}`)); got != "adapter_automation_error" {
