...
func IndicatorSources(cap Capabilities) []string {
	...
	for _, model := range cap.Models {
		if model.Provider != "" && !strings.EqualFold(model.Provider, "adapter") {
			seen["local"] = true
		}
	}
	order := []string{"adapter", "local"}
...
// badges. An unreported mode stays gray in the UI; no adapter feature is
