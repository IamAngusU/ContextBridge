...
func TestSubmitHTTPRejectsMalformedUTF8AndUnsafeUnicodeLabels(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "provider isolate", requirements: Requirements{Task: "generation", Provider: "browser\u2066spoof"}},
...
func FuzzValidRoutingLabelContract(f *testing.F) {
	for _, seed := range []string{"model", "GPT-5.6 Sol", "模型", strings.Repeat("é", 80), " safe", "safe ", "safe\nspoof", "safe\u202espoof", "safe\u2028spoof", string([]byte{0xff})} {
		f.Add(seed)
	}
	f.Fuzz(func(t *testing.T, value string) {
