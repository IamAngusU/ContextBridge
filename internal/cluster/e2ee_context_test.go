...
func TestE2EEAADRejectsEveryExecutionContextMutation(t *testing.T) {
	...
	job := Job{
		...
		TenantID: response.Assignment.TenantID, Requirements: response.Assignment.Requirements, SealedPayload: envelope,
	}
...
func TestWorkerFailsBeforeLocalExecutionWhenEncryptedMetadataIsTampered(t *testing.T) {
	...
	encryptionContext := EncryptionContext{
		...
		Requirements: Requirements{Task: "generation", Provider: "browser", SessionID: "session-a"},
	}
	...
	defer local.Close()
	worker := &Worker{
		cfg:      WorkerConfig{LocalURL: local.URL, LocalToken: "local-token", AllowedTasks: []string{"generation", "embedding"}, AllowedProviders: []string{"browser"}},
