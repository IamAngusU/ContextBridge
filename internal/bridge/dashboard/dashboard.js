...
function updateServiceClock() {
  ...
  $('server-clock').textContent=`${local} ${offsetLabel(serviceClock.offset)} · ${signedDelay(serviceClock.delta)} vs this viewer (±${Math.round(serviceClock.uncertainty)} ms)`;
}
...
async function createSchedule(event) {
  ...
  job.metadata={contextbridge_foreground_new_session:values.has('foreground')};
  if (values.has('new-session')) job.metadata.contextbridge_new_session_per_run=true;
...
function renderAdapter(adapter) {
  ...
  $('adapter-metric').textContent = connected ? `${adapter.active_endpoints || endpoints.length} endpoint(s) · ${adapter.busy_endpoints || 0} busy` : 'Not connected';
  $('adapter-detail').textContent = connected
    ? `${adapter.adapter || 'adapter'} ${adapter.adapter_version || ''}`.trim()
    : 'No optional adapter connected';
  $('endpoint-title').textContent = endpoints.map((endpoint) => `Endpoint ${endpoint.id || '?'}`).join(' · ') || 'No endpoint connected';
  $('endpoint-driver').textContent = adapter.adapter || 'None';
  $('endpoint-profile').textContent = endpoints.map((endpoint) => `${endpoint.profile || 'adapter'}${endpoint.current_model ? `: ${endpoint.current_model}` : ''}`).join(' · ') || adapter.profile_label || 'None';
  $('endpoint-state').textContent = connected ? (adapter.state || 'waiting') : 'paused';
  $('endpoint-seen').textContent = meaningfulTimestamp(adapter.last_seen) ? relativeTime(adapter.last_seen) : 'Never';
}
...
function renderRoutes(routes) {
  ...
  for (const [name, route] of Object.entries(routes).sort(([a], [b]) => a.localeCompare(b))) {
    ...
    fallback.textContent = `${chain.join('  >  ')}${route.adapter_profile ? `  |  ${route.adapter_profile}` : ''}`;
