...
func TestScheduleClaimRestartNeverReplays(t *testing.T) {
	...
	now := time.Now().UTC()
	item := Schedule{ID: "safe", Name: "safe", Job: Job{Prompt: "hello", Model: "Adapter Model A", Reasoning: "Sehr hoch"},
	...
	if claimed.CurrentRunID != job.ID || job.Metadata["contextbridge_reasoning_fallbacks"] == nil || job.Metadata["contextbridge_new_session"] != true || job.Metadata["contextbridge_foreground_new_session"] != true || !claimed.NextRun.After(now) {
...
func TestScheduleFileHandoffRequiresVerifiedBytes(t *testing.T) {
	cfg := config.Config{Storage: config.Storage{Directory: t.TempDir(), Inbox: t.TempDir()}, Routes: map[string]config.Route{"default": {Provider: "adapter", AdapterProfile: "profile-one", TimeoutSeconds: 5}}}
	...
	base := Job{ID: "schedule-test-run", Route: "default", SessionID: "schedule-test", Metadata: map[string]interface{}{"contextbridge_foreground_new_session": true}}
...
func TestScheduleLocalImageHandoffPreservesVerifiedProvenance(t *testing.T) {
	cfg := config.Config{
		...
		Routes: map[string]config.Route{
			"default": {Provider: "adapter", AdapterProfile: "profile-one", TimeoutSeconds: 5},
...
func TestScheduleAPIDoesNotInheritAdapterSelectionAcrossLocalRoute(t *testing.T) {
	cfg := config.Config{
		Server:  config.Server{Token: "test-token"},
		Storage: config.Storage{Directory: t.TempDir(), Inbox: t.TempDir()},
		Routes: map[string]config.Route{
			"adapter": {Provider: "adapter", AdapterProfile: "profile-one", TimeoutSeconds: 5},
	...
	payload := map[string]interface{}{
		"name": "adapter to local vision",
		"job": map[string]interface{}{
			"route": "adapter", "provider": "adapter", "model": "remote explicit", "reasoning": "high", "prompt": "Create one image.",
	...
	if local.Provider != "ollama" || local.Model != "" || local.Reasoning != "" || local.Route != "local" || local.Task != "vision" || outputMode(local.Output) != "text" {
		t.Fatalf("adapter selection leaked into local follow-up: %+v", local)
	}
}

func TestScheduleAPIAndStatusRedaction(t *testing.T) {
	cfg := config.Config{Server: config.Server{Token: "test-token"}, Storage: config.Storage{Directory: t.TempDir(), Inbox: t.TempDir()}, Routes: map[string]config.Route{"default": {Provider: "adapter", AdapterProfile: "profile-one", TimeoutSeconds: 5}}}
	server, err := NewServer(cfg, log.New(io.Discard, "", 0))
	if err != nil {
		t.Fatal(err)
	}
	...
	server.schedules.waiting(item.ID, "adapter_disconnected")
	server.dispatchSchedules(t.Context())
	loaded, _ = server.schedules.get(item.ID)
	if loaded.CurrentRunID != "" {
		t.Fatal("disconnected adapter schedule was dispatched")
	}
...
func TestOutcomeBreakdownUsesRequestedSelection(t *testing.T) {
	...
	job := Job{ID: "one", Route: "default", Provider: "adapter", Model: "Adapter Model A", Reasoning: "Sehr hoch"}
	store.RecordCompleted(job, Output{Mode: "text", Provider: "adapter", Model: "adapter", SelectedModel: "Adapter Model B", SelectedReasoning: "Hoch", Error: "adapter_timeout"})
	metrics := store.Metrics()
	if metrics.JobsFailed != 1 || metrics.ByAttemptedProvider["adapter"] != 1 || metrics.ByAttemptedModel["Adapter Model A"] != 1 || metrics.ModelFailures["Adapter Model A"] != 1 || metrics.ReasoningFailures["Sehr hoch"] != 1 || metrics.SelectionFailures["adapter / Adapter Model A / Sehr hoch"] != 1 {
