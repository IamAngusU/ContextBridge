...
func TestImageSeriesUsesOneAggregateBudgetAndCompletesAtomically(t *testing.T) {
	...
	exact := NormalizeOutput(imageSeriesEnvelope(t, exactSizes), spec, "browser", "test-tab", time.Millisecond)
	...
	over := NormalizeOutput(imageSeriesEnvelope(t, []int{341, 341, 343}), spec, "browser", "test-tab", time.Millisecond)
...
func TestImageSeriesCountCapIsDeterministic(t *testing.T) {
	...
	output := NormalizeOutput(raw, spec, "browser", "test-tab", time.Millisecond)
