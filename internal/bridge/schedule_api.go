	...
	case http.MethodPost:
		...
		if len(input.Fallback.Models) > 0 || len(input.Fallback.Reasoning) > 0 {
			...
			for _, provider := range providers {
				if engine, ok := s.cfg.Engine(provider); ok && engine.Type == "adapter" {
					adapterAllowed = true
				}
			}
			if !adapterAllowed {
				writeJSON(w, http.StatusUnprocessableEntity, map[string]string{"error": "adapter menu alternatives require a adapter provider"})
				return
			}
		}
		...
		for index := range input.Steps {
			...
			if step.UsePreviousArtifact != "" {
				...
				if engine.Type != "adapter" && (step.UsePreviousArtifact != "image" || (engine.Type != "ollama" && engine.Type != "llama_cpp")) {
